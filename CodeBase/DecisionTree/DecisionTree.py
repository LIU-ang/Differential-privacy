...
from lib import LaplaceMechanism
...
# will divide current data into multiple parts by vals
...
def makeTree(data, attributes, attributesType, target, depth, recursion, epsilon = 0):
    # logging.debug('building on depth ', recursion)
...
def classify(tree, attributes, attributesType, query):
    ...
    for entry in query:
        ...
        while (not tmpTree.has_key('leaf')):
            attr = tmpTree['attr']
            index = attributes.index(attr)
            type = attributesType[index]
            value = entry[index]
